...
However, it's best to create your own `yaml` configuration file and modify the configuration file regarding to your needs.
...
```
npx @apideck/portman@latest --local openapi.yaml --portmanConfigFile portman-test-config.yaml --output postmanCollection.json
```

`openapi.yaml` is the name of the OpenAPI file, `portman-test-config.yaml` is the name of the configuration file for the tests, and `postmanCollection.json` is the name of the generated postman collection.
...
When we execute the tests for the first time, some of them will fail.
...
We will now modify our Portman test configuration `portman-test-config.yaml` to change the execution order and reuse the ID as variable.

First, we can use the `orderOfOperations` keyword in Portman to change the execution order. Add the following lines to your configuration on the top level at the bottom of the file:
```yaml
globals:
  orderOfOperations:
    - "POST::/*"
...
> [!IMPORTANT]  
> Ordering only works per tag and not globally!

We can also extract the ID from the entity created in the contract test by adding the following configuration on the top level at the bottom of the file:
```yaml
assignVariables:
...
We can add it to the globals object in the configuration file where we already added the `orderOfOperations`,
```yaml
globals:
...
Your complete file should look like this now:
```yaml
version: 1
tests:
  contractTests:
...
When we run the tests again now, we can check that the operation order is correctly applied and the `petId` is passed on to the next operations.
...
```
npx @apideck/portman --local openapi.yaml --portmanConfigFile portman-test-config.yaml --baseUrl https://petstore3.swagger.io/api/v3 --output postmanCollection.json
```
...
> [!NOTE]  
> Overwrites just work if the OpenAPI contains the respective security scheme which we want to overwrite.
...
For testing locally now, we can add our credentials directly to the `portman-test-config.yaml`.
When we share the file or want to use Portman in a pipeline, we should use secret variables and a `.env` file.
